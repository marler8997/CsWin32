...
";
        ...
        internal static void Generate(MetadataReader mr, string out_dir, CancellationToken cancel_token)
        {
            ...
            foreach (Api api in generator.api_namespace_map.Values)
            {
                ...
                using var out_stream_writer = new StreamWriter(file_stream, Encoding.UTF8);
                var out_file = new TabWriter(out_stream_writer);
        ...
        void generateApi(TabWriter out_file, Api api)
        {
            out_file.WriteLine(AutogeneratedMessage);
            out_file.WriteLine("");
            out_file.WriteLine("//");
            ...
            if (api.constants != null)
            {
                foreach (FieldDefinitionHandle field_def in api.constants)
                {
                    ...
                    this.generateConst(out_file, field_def);
                    const_count += 1;
                }
            }
        ...
        void generateConst(TabWriter out_file, FieldDefinitionHandle field_def_handle)
        {
            ...
            if (field_def.Attributes != expected)
            {
                throw new InvalidOperationException(string.Format(
                    "Expected Constant FieldDefinition to have these attributes '{0}' but got '{1}'", expected, field_def.Attributes));
            }
            assertData(field_def.GetOffset() == -1);
            assertData(field_def.GetRelativeVirtualAddress() == 0);

            // TODO: what is field_def.GetMarshallingDescriptor?
            foreach (CustomAttributeHandle attr_handle in field_def.GetCustomAttributes())
            {
                ConstantAttr attr = this.decodeConstantAttr(this.mr.GetCustomAttribute(attr_handle));
                if (object.ReferenceEquals(attr, ConstantAttr.Instance))
                {
                    // we lready assume "const" on all constant values where this matters (i.e. string literals)
                }
                else if (attr is ConstantAttr.NativeTypeInfo native_type_info)
                {
                    // we already assume null-termination on all constant string literals
                    assertData(native_type_info.unmanaged_type == UnmanagedType.LPWStr);
                    assertData(native_type_info.is_null_terminated);
                }
                else if (attr is ConstantAttr.Obsolete obsolete)
                {
                    out_file.WriteLine("// [Obselete(\"{0}\")] (TODO: can Zig do something with this?)", obsolete.value);
                }
                else
                {
                    assertData(false);
            ...
            string name = this.mr.GetString(field_def.Name);
        ...
        void generateType(TabWriter out_file, TypeGenInfoSet type_refs, TypeGenInfo type_info)
        {
            ...
            else
            {
                ...
                {
                    out_file.depth += 1;
                    this.generateType(out_file, type_refs, nested_type);
                    out_file.depth -= 1;
                }
                out_file.WriteLine("};");
            }
        ...
        bool generateTypeInner(TabWriter out_file, TypeGenInfoSet type_refs, TypeGenInfo type_info)
        {
            out_file.WriteLine("// --------------------------------------------------------");
            ...
            if (type_info.isNested())
            {
                assertData(attrs.visibility == TypeVisibility.nested_public);
            }
            else
            {
        ...
        ConstantAttr decodeConstantAttr(CustomAttribute attr)
        {
            ...
            if (attr_name.Equals("Windows.Win32.Interop", "ConstAttribute"))
            {
                enforceAttrFixedArgCount(attr_name, attr_args, 0);
                enforceAttrNamedArgCount(attr_name, attr_args, 0);
                return ConstantAttr.Instance;
            }
            if (attr_name.Equals("Windows.Win32.Interop", "NativeTypeInfoAttribute"))
            {
                enforceAttrFixedArgCount(attr_name, attr_args, 1);
                enforceAttrNamedArgCount(attr_name, attr_args, 1);
                UnmanagedType unmanaged_type = attrFixedArgAsUnmanagedType(attr_args.FixedArguments[0]);
                enforceNamedArgName(attr_name, attr_args, "IsNullTerminated", 0);
                bool is_null_terminated = attrNamedAsBool(attr_args.NamedArguments[0]);
                return new ConstantAttr.NativeTypeInfo(unmanaged_type, is_null_terminated);
            }
            if (attr_name.Equals("System", "ObsoleteAttribute"))
            {
                enforceAttrFixedArgCount(attr_name, attr_args, 1);
                enforceAttrNamedArgCount(attr_name, attr_args, 0);
                return new ConstantAttr.Obsolete(attrFixedArgAsString(attr_args.FixedArguments[0]));
            }
            throw new NotImplementedException(string.Format("uhandled constant custom attribute \"{0}\", \"{1}\"", attr_name.@namespace, attr_name.name));
        }
        ...
        BasicTypeAttr decodeBasicTypeAttr(CustomAttribute attr)
        {
            ...
            throw new NotImplementedException(string.Format("uhandled type custom attribute \"{0}\", \"{1}\"", attr_name.@namespace, attr_name.name));
        }
        ...
        string generateFunc(TabWriter out_file, TypeGenInfoSet type_refs, MethodDefinitionHandle func_handle)
        {
