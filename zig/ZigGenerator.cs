...
";
        ...
        private void GenerateApi(StreamWriter out_file, Api api)
        {
            ...
            {
                this.cancel_token.ThrowIfCancellationRequested();
                this.GenerateType(out_file, api, type_info);
                type_count += 1;
            }
            ...
            out_file.WriteLine("    @setEvalBranchQuota(constant_export_count + type_export_count + 2); // TODO: why do I need these extra 2?");
            out_file.WriteLine("    @import(\"std\").testing.refAllDecls(@This());");
            out_file.WriteLine("}}");
        }
        ...
        private void GenerateType(StreamWriter out_file, Api api, TypeGenInfo type_info)
        {
            out_file.WriteLine("// --------------------------------------------------------");
    ...
    internal class Api
    {
        ...
        public readonly List<TypeGenInfo> types = new List<TypeGenInfo>();
        public readonly Dictionary<string, TypeGenInfo> type_name_map = new Dictionary<string, TypeGenInfo>();
        ...
        internal void addType(TypeGenInfo type_info)
        {
            ...
            this.type_name_map.Add(type_info.name, type_info);
        }

        internal AddedTypeRef addTypeRef(TypeRef type_ref)
        {
            type_ref.addImports(this.type_imports);
            return new AddedTypeRef(this, type_ref);
        }

        // A TypeRef whose usage has been "added" to the api
        internal struct AddedTypeRef
        {
            public readonly Api api_context;
            public readonly TypeRef type_ref;
            public readonly string zig_str;

            internal AddedTypeRef(Api api_context, TypeRef type_ref)
            {
                this.api_context = api_context;
                this.type_ref = type_ref;
                StringBuilder builder = new StringBuilder();
                type_ref.formatZigType(builder);
                this.zig_str = builder.ToString();
            }
        }
    }
