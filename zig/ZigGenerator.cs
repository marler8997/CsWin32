...
";
        ...
        private readonly Dictionary<string, Api> namespace_to_api_map = new Dictionary<string, Api>();
        ...
        private ZigGenerator(MetadataReader mr, CancellationToken cancel_token)
        {
            ...
            foreach (TypeDefinitionHandle type_def_handle in this.mr.TypeDefinitions)
            {
                ...
                else if (type_info.@namespace.StartsWith(WindowsWin32NamespacePrefix, StringComparison.Ordinal))
                {
                    api_name = type_info.@namespace.Substring(WindowsWin32NamespacePrefix.Length);
                }
                else
                {
                    throw new InvalidDataException(string.Format(
                        "Type '{0}' is not in the Windows.Win32 namespace, it is in '{1}'",
                        type_info.name,
                        type_info.@namespace));
                }
                ...
                {
                    Api? api;
                    if (!this.namespace_to_api_map.TryGetValue(api_name, out api))
                    {
                        api = new Api(api_name);
                        this.namespace_to_api_map.Add(api_name, api);
                    }
        ...
        public static void Generate(MetadataReader mr, string out_dir, CancellationToken cancel_token)
        {
            ...
            foreach (Api api in generator.namespace_to_api_map.Values)
            {
...

            {
                ...
                out_file.WriteLine(@"    const api_count = {0};", generator.namespace_to_api_map.Count);
